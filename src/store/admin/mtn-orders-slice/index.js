...
// Async thunk to fetch all MTN orders
export const fetchAllMTNOrders = createAsyncThunk(
  'mtnOrders/fetchAll',
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP ${response.status}: Failed to fetch MTN orders`);
      }
      ...
      if (result.success) {
        console.log(`✅ Redux: Successfully loaded ${result.data.length} MTN orders`);
        return result.data;
      } else {
        console.error('❌ Redux: API returned failure:', result.message);
        throw new Error(result.message || 'Failed to fetch MTN orders');
      }
    } catch (error) {
      console.error('❌ Redux: Fetch MTN orders error:', error);
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      console.log('📡 Redux: All orders API response:', {
        success: result.success,
        mtnOrdersLength: result.data?.length,
        allOrdersCount: result.allOrdersCount,
        message: result.message
      });

      if (result.success) {
        console.log(`✅ Redux: Successfully loaded ${result.data.length} MTN orders from ${result.allOrdersCount} total orders`);
        return result.data;
      } else {
...
export const updateOrderStatus = createAsyncThunk(
  ...
  async ({ orderId, status }, { rejectWithValue }) => {
    try {
      ...
      if (result.success) {
        console.log(`✅ Redux: Order ${orderId} status updated to ${status}`);
        return { orderId, status, order: result.data };
      } else {
        throw new Error(result.message || 'Failed to update order status');
      }
    } catch (error) {
      console.error('❌ Redux: Update order status error:', error);
      return rejectWithValue(error.message);
    }
  }
);
...
export const bulkUpdateOrderStatus = createAsyncThunk(
  ...
  async ({ orderIds, status }, { rejectWithValue }) => {
    try {
      ...
      if (result.success) {
        console.log(`✅ Redux: Bulk updated ${result.data.updatedCount} orders`);
      ...
      console.error('❌ Redux: Bulk update order status error:', error);
      return rejectWithValue(error.message);
    }
  }
);

const mtnOrdersSlice = createSlice({
  name: 'mtnOrders',
  initialState: {
    orders: [],
    loading: false,
  ...
  extraReducers: (builder) => {
    builder
      // Fetch all MTN orders
      .addCase(fetchAllMTNOrders.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchAllMTNOrders.fulfilled, (state, action) => {
        state.loading = false;
        state.orders = action.payload;
        state.error = null;
        state.lastFetch = new Date().toISOString();
      })
      .addCase(fetchAllMTNOrders.rejected, (state, action) => {
        state.loading = false;
...
export const { clearError, setOrders } = mtnOrdersSlice.actions;
export default mtnOrdersSlice.reducer;