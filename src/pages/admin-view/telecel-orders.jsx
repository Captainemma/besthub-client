...
import { Search, Download, Calendar, Zap, CheckCircle, XCircle, Clock, CheckSquare, Square, X, RefreshCw } from "lucide-react";
import { useState, useEffect, useCallback, useMemo } from "react";
import { useSelector, useDispatch } from "react-redux";
import { 
  fetchAllTelecelOrders, 
  updateOrderStatus, 
...
function TelecelOrders() {
  ...
  const telecelOrdersState = useSelector((state) => state.telecelOrders);
  const { 
    orders = [], 
    loading, 
    updating, 
  ...
  useEffect(() => {
    console.log('🔍 Telecel Orders State:', telecelOrdersState);
    console.log('🔍 Orders data:', orders);
  }, [telecelOrdersState, orders]);

  // Fetch orders on component mount
  useEffect(() => {
    console.log('🔄 Fetching Telecel orders...');
    dispatch(fetchAllTelecelOrders());
  }, [dispatch]);
    ...
    const testAPI = async () => {
      try {
        ...
        if (data.success && data.data) {
          console.log('✅ API returned orders:', data.data.length);
        }
      } catch (error) {
        console.error('❌ Telecel Orders API test failed:', error);
      }
    };
    testAPI();
  }, []);
  ...
  const handleSingleStatusChange = useCallback((orderId, newStatus) => {
    dispatch(updateOrderStatus({ orderId, status: newStatus }))
      .unwrap()
      .then(() => {
        console.log(`✅ Telecel order ${orderId} status updated to ${newStatus}`);
      })
      .catch(error => {
    ...
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h1 className="text-2xl font-bold text-gray-900">Telecel Orders Management</h1>
          <p className="text-gray-600">Loading Telecel orders data...</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Telecel Orders Management</h1>
          <p className="text-gray-600">View and manage all Telecel data bundle orders</p>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Telecel Orders</CardTitle>
        ...
        <CardContent>
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900 truncate">{order.package || 'Telecel Data Bundle'}</p>
          ...
          {filteredOrders.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-lg font-medium">No Telecel orders found</p>
              <p className="text-sm text-gray-500 mt-2 max-w-md mx-auto">
                {searchTerm || statusFilter !== 'all' 
                  ? 'Try adjusting your search terms or filters' 
                  : 'There are no Telecel orders in the system yet'
                }
              </p>
              {!searchTerm && statusFilter === 'all' && (
                <Button 
